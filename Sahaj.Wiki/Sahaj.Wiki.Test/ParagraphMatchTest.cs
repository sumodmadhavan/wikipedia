...
namespace Sahaj.Wiki.Test
{
    ...
    public class ParagraphMatchTest
    {
        ...
        {
            string[] sentences = sentencesString.Split('.');
            ...
            var result = paragraphMatching.AnalyzeText(sentences, questions, answers);
            Assert.AreEqual("Grévy's zebra and the mountain zebra", result[1]);
            Assert.AreEqual("aims to breed zebras that are phenotypically similar to the quagga", result[2]);
            Assert.AreEqual("horses and donkeys", result[3]);
            Assert.AreEqual("the plains zebra, the Grévy's zebra and the mountain zebra", result[4]);
            Assert.AreEqual("subgenus Hippotigris", result[5]);
        }
    }
}
